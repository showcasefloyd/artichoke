...
class ComicDB_Series extends ComicDB_Object
{
    ...
    protected function select()
    {
        ...
        $query              = "SELECT s.id, s.title, s.name, s.volume, s.start_year, p.name AS publisher, s.type,\n";
        if ($includeTotalIssues) {
            $query .= "       s.default_price, s.first_issue, s.final_issue, s.total_issues, s.subscribed, s.comments\n";
        } else {
            $query .= "       s.default_price, s.first_issue, s.final_issue, s.subscribed, s.comments\n";
        }
        $query .= ""
            . "  FROM series s\n"
    ...
    protected function insert()
    {
        ...
        $data['name']  = "'" . $this->name() . "'";
        $publisherName = $db->real_escape_string($this->publisher() ?? '');
        $pubResult     = $db->query("SELECT id FROM publisher WHERE name = '$publisherName' LIMIT 1");
        if ($pubResult && $pubRow = $pubResult->fetch_assoc()) {
            $data['publisher_id'] = (int) $pubRow['id'];
        }
    ...
    protected function update()
    {
        ...
        if ($titleId && (int) $titleId > 0) {
            $data['title'] = (int) $titleId;
        }
        ...
        if ($publisher) {
            ...
            $pubResult        = $db->query("SELECT id FROM publisher WHERE name = '$publisherEscaped' LIMIT 1");
            if ($pubResult && $pubRow = $pubResult->fetch_assoc()) {
                $data['publisher_id'] = (int) $pubRow['id'];
            }
        }
